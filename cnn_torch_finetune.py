...
try:
    ...
    print("Warning: gensim import failed:", str(e))
    print("如果想使用预训练 word2vec，请先执行: pip install gensim")
    _HAS_GENSIM = False
...
def build_model(config, vocab_size, num_classes, tokenizer=None):
    """
    更稳健的 load word2vec 流程：
      - 支持 config['word2vec_file'] 或 config.get('extra', {})['word2vec_file']
      - 检查文件存在性，并给出合理提示
      - 自动判断 binary 参数（.bin -> binary=True）
      - 为 padding(0) 保持全 0 向量，为 OOV(1) 使用随机向量或从模型采样
    """
    model_type = config.get('model_type', 'textcnn_w2v')

    if model_type != 'textcnn_w2v':
        raise ValueError(f"Unknown model type: {model_type}")

    # try to find word2vec path in top-level or in extra dict
    w2v_path = config.get('word2vec_file', None)
    if w2v_path is None and isinstance(config.get('extra', None), dict):
        w2v_path = config['extra'].get('word2vec_file', None)

    embedding_matrix = None
    embedding_dim = config.get('embedding_dim', 50)
    trainable = config.get('trainable', True)

    if w2v_path and _HAS_GENSIM:
        if not os.path.exists(w2v_path):
            print(f"Warning: 找不到 word2vec 文件: {w2v_path} (脚本当前工作目录: {os.getcwd()})")
            print("请确认路径正确或使用绝对路径。将回退到随机初始化。")
        else:
            try:
                print(f"Loading Word2Vec from {w2v_path} ... (this may take a while and require large RAM)")
                # detect binary by extension
                binary = w2v_path.lower().endswith('.bin')
                w2v = KeyedVectors.load_word2vec_format(w2v_path, binary=binary)
                embedding_dim = w2v.vector_size
                # allocate embedding matrix (vocab_size x embedding_dim)
                embedding_matrix = np.zeros((vocab_size, embedding_dim), dtype=np.float32)

                # ensure padding idx (0) stays zero-vector
                # fill OOV index (1) with small random vector
                rng = np.random.RandomState(SEED)
                if 1 < vocab_size:
                    embedding_matrix[1] = rng.normal(scale=0.6, size=(embedding_dim,))

                # fill known tokens
                # Note: tokenizer.word_index maps token->idx, idx starts from 1 (OOV) and 2...
                if tokenizer is not None:
                    for word, idx in tokenizer.word_index.items():
                        ...
                        if word in w2v:
                            ...
                            # try alternate strategies: lowercase lookup (only if tokenizer used lowercasing)
                            lw = word.lower()
                            if lw in w2v:
                                embedding_matrix[idx] = w2v[lw]
                            else:
                                # leave as random vector
                                embedding_matrix[idx] = rng.normal(scale=0.6, size=(embedding_dim,))
                else:
                    # no tokenizer: fill embeddings randomly except index 0
                    for i in range(1, vocab_size):
                        embedding_matrix[i] = rng.normal(scale=0.6, size=(embedding_dim,))

                print("Word2Vec loaded and embedding matrix prepared.")
            except Exception as e:
                print("加载 word2vec 失败，回退到随机初始化。错误：", str(e))
                embedding_matrix = None
    else:
        if not w2v_path:
            print("⚠️ 未提供 word2vec_file（config 中找不到），将使用随机初始化。")
        elif not _HAS_GENSIM:
            print("⚠️ gensim 不可用，将使用随机初始化。请执行: pip install gensim")
        embedding_matrix = None
